...
def findOutliers(cdns,types,country,rttDict):
	if country=="dnsUS_TRIN":
		myType="remote_non-neighboring_region"
	elif country=="dnsUS_TRAR":
		myType="remote_neighboring_region"
	ips=[]
	_outlierDict={}
...
def plotrttCDFs(cdns,resolvers,cdnized_domain_ips,region,filename,outliers_neigh={},outliers_non_neigh={}):
	...
	for cdn in cdns:
		...
		for resolver in resolvers:
			...
			fig.subplots_adjust(hspace=0)
			# fig.tight_layout()
			plt.savefig("graphs/sigcommRes/"+region+"/CDFs_"+filename+cdn)
			# plt.clf()
...
if __name__ == "__main__":
	...
	with open("results/sigcommRes/"+region+"/rtts/rttDict_allCDNs"+country+".json", 'w') as fp:
		json.dump(_dict, fp)		
	# outliersIN=findOutliers(cdns,types,country,_dict)
	# _dict={}
	# country="dnsUS_TRAR"
	# for cdn in cdns:
	...
	# with open("results/sigcommRes/"+region+"/rtts/rttDict_allCDNs"+country+".json", 'w') as fp:
	# 	json.dump(_dict, fp)		
	# outliersAR=findOutliers(cdns,types,country,_dict)

	######################################################last uncommented code
	rtt_dict=json.load(open("results/sigcommRes/"+region+"/rtts/rttDict_allCDNs.json"))
	# outliersAR=json.load(open("results/sigcommRes/"+region+"/outliers_dnsUS_TRAR.json"))
	# outliersIN=json.load(open("results/sigcommRes/"+region+"/outliers_dnsUS_TRIN.json"))
	# _dict={}
